...
const DiceRoller: React.FC<DiceRollerProps> = ({ check, worldRoll, onRollComplete, modifier = 0 }) => {
  ...
  const startRoll = useCallback(() => {
    if (spinRef.current || pauseRef.current) return;
    ...
    spinRef.current = setInterval(() => {
      ...
      if (elapsed % 100 === 0) SoundManager.playHover();
      ...
      if (elapsed >= ROLL_DURATION_MS) {
        ...
        const finalRoll = worldRoll?.result ?? Math.floor(Math.random() * 20) + 1;
        setCurrentFace(finalRoll);
        setResult(finalRoll);
        setIsRolling(false);

        if (finalRoll === 20) SoundManager.playNat20();
        else if (finalRoll === 1) SoundManager.playNat1();
        else SoundManager.playConfirm();

        pauseRef.current = setTimeout(() => {
          pauseRef.current = null;
          onRollCompleteRef.current(finalRoll);
        }, POST_ROLL_PAUSE_MS);
      }
    }, FRAME_INTERVAL_MS);
  }, [worldRoll]);
          ...
          {check && (
            <div className="flex items-center justify-center gap-6 text-sm font-black">
               ...
               <div className="flex flex-col items-center bg-slate-800/50 p-3 rounded-2xl min-w-[70px] border border-white/5">
                  <span className="text-slate-600 text-[10px] uppercase tracking-widest">Total</span>
